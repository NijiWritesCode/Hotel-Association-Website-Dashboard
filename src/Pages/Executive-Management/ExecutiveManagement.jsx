...
import { User, Workflow, Hotel, Phone, Image, CheckCircle, XCircle, X, MapPin } from 'lucide-react';
...
// 🔧 normalize image for older docs
const normalizeImage = (executive) => {
  if (!executive.profileImage) return null;
  return { url: executive.profileImage, base64: executive.profileImage, file: null };
};
  ...
  new Promise((resolve) => {
    ...
    reader.onloadend = () => {
      ...
      img.onload = () => {
        ...
        const maxSize = 500; // 🔧 resize threshold
...
const ExecutiveManagement = () => {
  ...
  const handleImageChange = async (e) => {
    ...
    if (file) {
      ...
      setSelectedImage({ base64, url: base64, file });
    }
  };
  ...
  const fetchExecutives = async () => {
    try {
      ...
      const executiveList = querySnapshot.docs.map((docSnap) => {
  ...
  const addExecutive = async (e) => {
    ...
    // ✅ only overwrite if user selects a new image
    let imageBase64 = editingId
  ...
  const editExecutive = (executive) => {
    ...
    setSelectedImage(normalizeImage(executive));
  ...
  return (
    <>
      ...
      {notification && (
        <div className={`notification ${notification.type}`}>
          <div className="notification-content">
            ...
            <button className="notification-close" onClick={dismissNotification} aria-label="Close notification">
              <X size={16} />
            </button>
          </div>
        </div>
      ...
      <ConfirmationModal isOpen={modal.isOpen} onConfirm={modal.onConfirm} onCancel={closeModal} message={modal.message} />
        ...
        <div className="exec-form-container">
          ...
          <div className="exec-form-fields">
            ...
            <label htmlFor="image" className="form-label">
              ...
              <div className="input-wrapper">
                <input type="file" name="image" accept="image/*" onChange={handleImageChange} className="input-field" />
          ...
          {selectedImage && (
            <div className="image-preview">
              ...
              <img src={selectedImage.url} alt="Selected profile" className="preview-image" />
            </div>
          )}
        ...
        {loading ? (
          ...
          <div className="executive-list">
            {executives.slice(0, 20).map((executive) => (
              <ExecutiveCard key={executive.id} executive={executive} onDelete={deleteExecutive} onEdit={editExecutive} />
            ))}
          </div>
        )}
      </div>
